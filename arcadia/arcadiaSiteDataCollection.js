...
alterState(state => {
  const sitesMap = { //swm site lookup table to ProjectIDs
    ...
    bangladesh_myanmar_coast: '1789',
    batang_ai_complex: '1790',
    bolivian_highlands: '1791',
    bolivian_piedmont: '1792',
    bosque_valdiviano___parques_patagonia___pn_p_moreno: '1793',
    ...
    kafue_complex: '1819',
    karimunjawa: '1820',
    karukinka___coraz_n_de_la_isla___pla__mitre___ide: '1821',
    la_amistad: '1822',
    ...
    ontario_s_northern_boreal: '1911',
  ...
  const cameraTrapMap = { //lookup table for camera trap collection types
    still_images_will_be_collected: 1, //kobo_value: sample Ids for reference records
...
upsert('WCSPROGRAMS_ProjectAnnualDataPlan', 'DataSetUUIDID', { //TODO: Consider what other columns to map or mark not null in db
  ...
  //WCSToOpenFN: See below, we can dro this one line 
  //formName: dataValue('$.formName'), //TODO: Capture the source survey name? 
  //WCSToOpenFN: Always set new UserID_X fields to be "SELECT UserID from Users WHERE Email= <Email address of submitter>" and if this does return a value, it should be -1 
  //UserID_CR: ?  //TODO: Other "not null" column mappings to consider? 
  //UserID_LM: ?
  //WCSToOpenFN: Set these to be 'OpenFN:' + formName 
  //CRIPAddress: ?
  //LMIPAddress: ? 
          ...
          dataCollectionTools.map(dct => {
            return {
              ...
              WCSPROGRAMS_ProjectAnnualDataPlanDataSetID: //FK to WCSPROGRAMS_ProjectAnnualDataPlanDataSetID
                dataValue('body._id') + dataset['datasets/survey_type'],
              IsForCollect: 1,
              WCSPROGRAMS_DataToolID: state.dataToolMap[dct],
            };
          })
      ),
      //TODO: Update job mappings if config for this WCSPROGRAMS_ProjectAnnualDataPlanDataSetDataTool table changes
